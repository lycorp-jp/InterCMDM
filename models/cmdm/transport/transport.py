...
class Transport:
    ...
    def sample(self, x1, prefix_lengths=None, same_length_noise=False):
        """Sampling x0 & t based on shape of x1 (if needed)
          Args:
            x1 - data point; [batch, *dim]
            prefix_lengths - optional prefix lengths for conditioning
            same_length_noise - if True, use same noise and timesteps for both halves (person A and B)
        """
        ...
        if self.is_causal:
            ...
            t = t.reshape(
                    t.shape[0], -1, self.num_frame_per_block)
            t[:, :, 1:] = t[:, :, 0:1]
        ...
        # If same_length_noise is True, make both halves have the same timesteps
        if same_length_noise:
            half_len = x1.shape[1] // 2
            t[:, half_len:] = t[:, :half_len]

        t = t.to(x1)
        if prefix_lengths is not None:
            for i in range(x1.shape[0]):
                t[i, :prefix_lengths[i]] = 1.0
        return t, x0, x1
    ...
    def training_losses(
        ...
        t, x0, x1 = self.sample(x1, prefix_lengths, same_length_noise)
        ...
        if self.model_type == ModelType.VELOCITY:
            ...
            if use_contrastive_loss:
                ...
                terms['loss'] += - mean_flat(((model_output - ut_tlide) ** 2), dim=dim) * 0.05
            if ae is not None:
