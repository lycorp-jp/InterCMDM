...
class Geometric_Losses:
    def __init__(self, recons_loss, joints_num, dataset_name, device):
        ...
        if self.dataset_name in ('interhuman', 'joint'):
            # joint shares the same interhuman layout (262D with foot contact)
            ...
            raise ValueError(f'Unsupported dataset_name {self.dataset_name} for Geometric_Losses')
    ...
    def calc_bone_lengths(self, motion):
        if self.dataset_name in ('interhuman', 'joint'):
            ...
            raise ValueError(f'Unsupported dataset_name {self.dataset_name} for calc_bone_lengths')
        bones = []
    ...
    def forward(self, motions, pred_motion):
        if self.dataset_name in ('interhuman', 'joint'):
            ...
            raise ValueError(f'Unsupported dataset_name {self.dataset_name} for Geometric_Losses.forward')
...
class Inter_Losses:
    def __init__(self, recons_loss, joints_num, dataset_name, device):
        ...
        if self.dataset_name in ('interhuman', 'joint'):
            ...
            raise ValueError(f'Unsupported dataset_name {self.dataset_name} for Inter_Losses')
    ...
    def forward(self, motion1, motion2, pred_motion1, pred_motion2):
        ...
        if self.dataset_name in ('interhuman', 'joint'):
            ...
            raise ValueError(f'Unsupported dataset_name {self.dataset_name} for Inter_Losses.forward')
