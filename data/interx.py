...
class MotionDatasetV2HHI(data.Dataset):
    def __init__(self, opt, split_file, motion_file, normalize=False):
        self.opt = opt
        self.motion_rep = opt.motion_rep

        if opt.motion_rep == "smpl":
            self.normalize = normalize
            self.normalizer = InterxNormalizerTorchSMPL()
        elif opt.motion_rep == "global":
            self.normalize = normalize
            self.normalizer = InterxNormalizerTorch()
        else:
            raise ValueError(f"Unsupported motion representation: {opt.motion_rep}")
...
class Text2MotionDatasetV2HHI(data.Dataset):
    def __init__(self, opt, split_file, w_vectorizer, motion_file, normalize=False):
        self.opt = opt
        self.motion_rep = opt.motion_rep
    ...
    def __getitem__(self, item):
        ...
        motion_1, motion_2 = motion[..., :motion.shape[-1]//2], motion[..., motion.shape[-1]//2:]

        if m_length < self.max_motion_length:
            motion_1 = torch.cat([motion_1,
                                torch.zeros((self.max_motion_length - m_length, motion_1.shape[1], motion_1.shape[2]))
                                ], dim=0)
            motion_2 = torch.cat([motion_2,
                                torch.zeros((self.max_motion_length - m_length, motion_2.shape[1], motion_2.shape[2]))
                                ], dim=0)
        else:
            motion_1 = motion_1[:self.max_motion_length]
            motion_2 = motion_2[:self.max_motion_length]
            m_length = self.max_motion_length

        if self.normalize:
            motion_1 = self.normalizer.forward(motion_1)
            motion_2 = self.normalizer.forward(motion_2)

        motion = torch.cat([motion_1, motion_2], axis=-1)
