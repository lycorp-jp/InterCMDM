...
from utils.coordinate import extract_coordinate, inv_transform
from utils.coordinate import rigid_transform as rigid_transform_torch
import codecs as cs


def get_features(motion1, motion2, feature_dim=262):
    x1 = motion1[..., :feature_dim]
    x2 = motion2[..., :feature_dim]
    return x1, x2
...
class InterHumanMotion(data.Dataset):
    def __init__(self, opt, normalize=True):
        ...
        self.normalizer = MotionNormalizer(feature_dim=self.feature_dim)
        ...
        for root, dirs, files in os.walk(pjoin(opt.data_root)):
            for file in tqdm(files):
                if file.endswith(".npy") and "person1" in root:
                    ...
                    motion1, motion2 = get_features(motion1, motion2, self.feature_dim)
                    motion1_swap, motion2_swap = get_features(motion1_swap, motion2_swap, self.feature_dim)

                    if np.isnan(motion1).any() or np.isnan(motion1_swap).any() or np.isnan(motion2).any() or np.isnan(motion2_swap).any():
                        continue
                    if np.isinf(motion1).any() or np.isinf(motion1_swap).any() or np.isinf(motion2).any() or np.isinf(motion2_swap).any():
                        continue
                    
                    if motion1.shape[0] < opt.window_size:
                        continue

                    self.data.append(motion1)
                    self.data.append(motion1_swap)
                    self.lengths.append(motion1.shape[0] - opt.window_size)
                    self.lengths.append(motion1_swap.shape[0] - opt.window_size)
                    self.data.append(motion2)
                    self.data.append(motion2_swap)
                    self.lengths.append(motion2.shape[0] - opt.window_size)
                    self.lengths.append(motion2_swap.shape[0] - opt.window_size)
...
class InterHumanMotionTest(data.Dataset):
    def __init__(self, opt, normalize=True):
        self.opt = opt
        self.normalize = normalize
        self.feature_dim = opt.feature_dim
        ...
        for root, dirs, files in os.walk(pjoin(opt.data_root)):
            for file in tqdm(files):
                if file.endswith(".npy") and "person1" in root:
                    ...
                    self.data.append([motion1, motion2])
                    self.lengths.append(motion1.shape[0])
                    self.motion_ids.append(motion_name)
    ...
    def __getitem__(self, item):
        motion1, motion2 = self.data[item]

        motion1 = self.normalizer.forward(motion1)
        motion2 = self.normalizer.forward(motion2)

        return motion1, motion2, self.motion_ids[item]


class InterHumanDataset(data.Dataset):
    def __init__(self, opt, normalize=True, use_interaction_features=True):
        self.opt = opt
        self.normalize = normalize
        self.feature_dim = opt.feature_dim
        self.normalizer = MotionNormalizer(feature_dim=self.feature_dim)
        self.max_cond_length = 1
        self.min_cond_length = 1
        self.max_gt_length = self.opt.max_motion_length
    ...
    def __getitem__(self, item):
        ...
        if self.opt.mode == "train" and np.random.rand() > 0.5:
            gt_motion1, gt_motion2 = gt_motion2, gt_motion1

        return 0, text, gt_motion1, gt_motion2, gt_length
